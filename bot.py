...
def has_access(user_id):
    ...
    return True, "У вас Базовый тариф (3 бесплатные карты в день)", False
...
def admin_callback(call):
    ...
    if call.data == "admin_users":
        ...
        for u in users:
            ...
            status = "❌ Истек"
            try:
                if sub and isinstance(sub, str) and datetime.datetime.fromisoformat(sub) > now:
                    status = f"✅ Платная подписка (до {sub[:10]})"
                elif trial and isinstance(trial, str) and datetime.datetime.fromisoformat(trial) > now:
                    status = f"⏳ Пробный (до {trial[:10]})"
            except Exception as e:
                status = f"⚠️ Ошибка БД: {e}"
...
if __name__ == '__main__':
    bot.set_my_commands([
        ...
        telebot.types.BotCommand("/profile", "👤 Мой профиль и статус подписки"),
